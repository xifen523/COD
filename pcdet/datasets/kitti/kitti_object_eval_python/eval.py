...
def clean_data(gt_anno, dt_anno, current_class, difficulty, metric):
    ...
    for i in range(num_dt):
        ...
        if metric == 0:  # eval bbox
            height = abs(dt_anno["bbox"][i, 3] - dt_anno["bbox"][i, 1])
        else:
            height = 100  # ignore bbox height,  when eval bev,3d，
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, metric):
    # metric: eval type. 0: bbox, 1: bev, 2: 3d
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, metric)
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, metric)
